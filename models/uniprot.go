...
var UniprotWhere = struct {
	...
	Accession: whereHelperstring{field: "\"uniprot\".\"accession\""},
	SeqhashID: whereHelperstring{field: "\"uniprot\".\"seqhash_id\""},
...
var (
	uniprotAllColumns            = []string{"accession", "seqhash_id", "database"}
	uniprotColumnsWithoutDefault = []string{"accession", "seqhash_id", "database"}
	uniprotColumnsWithDefault    = []string{}
	uniprotPrimaryKeyColumns     = []string{"accession"}
)

type (
	// UniprotSlice is an alias for a slice of pointers to Uniprot.
	// This should generally be used opposed to []Uniprot.
...
func (o *Uniprot) Seqhash(mods ...qm.QueryMod) seqhashQuery {
...
func (uniprotL) LoadSeqhash(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUniprot interface{}, mods queries.Applicator) error {
	var slice []*Uniprot
	var object *Uniprot
...
// SetSeqhash of the uniprot to the related item.
// Sets o.R.Seqhash to related.
// Adds o to related.R.Uniprots.
func (o *Uniprot) SetSeqhash(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Seqhash) error {
	var err error
	if insert {
...
func Uniprots(mods ...qm.QueryMod) uniprotQuery {
	...
	return uniprotQuery{NewQuery(mods...)}
}

// FindUniprot retrieves a single record by ID with an executor.
// If selectCols is empty Find will return all columns.
func FindUniprot(ctx context.Context, exec boil.ContextExecutor, accession string, selectCols ...string) (*Uniprot, error) {
	uniprotObj := &Uniprot{}
	...
	query := fmt.Sprintf(
		"select %s from \"uniprot\" where \"accession\"=$1", sel,
	)
...
func (o *Uniprot) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		cache.query = fmt.Sprintf("UPDATE \"uniprot\" SET %s WHERE %s",
			...
			strmangle.WhereClause("\"", "\"", len(wl)+1, uniprotPrimaryKeyColumns),
		)
...
func (o UniprotSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"uniprot\" SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), len(colNames)+1, uniprotPrimaryKeyColumns, len(o)))
...
func (o *Uniprot) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"uniprot\"", updateOnConflict, ret, update, conflict, insert)
...
func (o *Uniprot) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"uniprot\" WHERE \"accession\"=$1"
...
func (o UniprotSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"uniprot\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, uniprotPrimaryKeyColumns, len(o))
...
func UniprotExists(ctx context.Context, exec boil.ContextExecutor, accession string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"uniprot\" where \"accession\"=$1 limit 1)"
